            ...
            <div class="role" >
              <div class="role-header">
                <div class="hea">
                  ...
                  <ooredoo-edit-role-popup :role="role" :editRolePopupIsShown="!editRolePopupIsShown"  @cancelFunction="()=>{editRolePopupIsShown = !editRolePopupIsShown}" />
...
import OoredooEditRolePopup from "@/views/ooredoo/Roles/ooredooEditRolePopup.vue";
import {localStorageToken, localStorageUser} from "@/utils/localStorageData";
...
const companyId = localStorageUser.role.id_entreprise;
onMounted(async () => {
  try {
    const response = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/user/role/${localStorageUser.id_user}`);
    instance.value = response.data.data;
    const responseemployees = await axios.get('https://pfe.ramzi-issiakhem.com/api/v1/company/ooredoo/users');
...
async function saveRole() {
  try {
    ...
    const response = await axios.post(`https://pfe.ramzi-issiakhem.com/api/v1/company/role/add/${companyId}`, data, {headers: {'Authorization':`Bearer ${localStorageToken}`}});
