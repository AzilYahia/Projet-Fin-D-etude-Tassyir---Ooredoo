<template>
  <div class="sala">
    ...
    <div class="actions-square" @click="toggleMenu" v-show="isMenuOpen">
      <font-awesome-icon icon="fas fa-pencil" class="actions-square-icon" @click="editAffiliation(employee)"/>

      <font-awesome-icon icon="fas fa-trash-can" class="actions-square-icon" @click="deleteAffiliation(employee)" />


    </div>


    <EditAffiliationPopUp  :employee="selectedEmployee" v-if="selectedEmployee !== null" @cancelFunction="()=>{editEmployeePopupisShown = !editEmployeePopupisShown;}" :isShown="editEmployeePopupisShown" />

<!--    <AffiliationInfoPopUp  :employee="selectedEmployee"-->
...
import EditAffiliationPopUp from "@/views/ooredoo/Affiliation/EditAffiliationPopUp.vue";
...
async function deleteAffiliation(employee) {
  try {

    const deleteUrl = `https://pfe.ramzi-issiakhem.com/api/v1/oredoo/user/unassign/${employee.id_ooredoo_user}/${employee.role.id_entreprise}`;
    const response = await axios.delete(deleteUrl);

    // Handle the response as needed
    console.log(response.data);

    alert(employee.nom + ' ' + employee.prenom + 'successfully unassigned.');
...
function editAffiliation(employee) {
  selectedEmployee.value = employee;
  isShown.value = true;
