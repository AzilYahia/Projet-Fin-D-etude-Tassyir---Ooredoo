...
<!--      </button>-->
    ...
    <table v-if="loading"  cellspacing="0" >
      ...
      <tfoot>
      <tr>
        <td colspan="3" class="end-of-results" >
          End of results.
        </td>
      </tr>
...
<!--      <tbody v-for="oneInstance in filteredInstance">-->
      <tbody v-for="(oneInstance,userCompany) in (filteredInstance)">
      <tr>
<!--        <td>{{ oneInstance.email }}</td>-->
        <td>{{oneInstance?.prenom}}{{oneInstance?.nom}}</td>
<!--        <td>{{ oneInstance.nationalite }}</td>-->
        <td>{{userData[userCompany].nom}}</td>
        <td><wx-affiliation-ooredoo :employee="oneInstance"/></td>
      </tr>
      </tbody>
      <tfoot>
      <tr>
...
import WxAffiliationOoredoo from "@/views/ooredoo/Affiliation/wxAffiliationOoredoo.vue";
...
onMounted(async () => {
  try {
    const response = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/company/ooredoo/users`);
    instance.value = response.data.data;
    console.log(instance.value)
    const usersResponse = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/companies/client?perPage=50&page=1`);
...
const filteredInstance = computed(() => {
  return instance.value.filter(item => {
    return item.prenom.toLowerCase().includes(search.value.toLowerCase())
  })
})
