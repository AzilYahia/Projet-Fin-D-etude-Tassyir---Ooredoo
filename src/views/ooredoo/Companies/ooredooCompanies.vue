<template>
  <div class="whole">
    <div class="control-board">
      <div class="search">
        ...
        <input type="text" placeholder="Search Companies" class="search-field" v-model="search"/>
      </div>
      <button class="add" @click="createClientCompany()">
    ...
    <table v-else cellspacing="0" >
      ...
      <tr>
        <td>{{ oneInstance.nom }}</td>
        <td>{{oneInstance.etat}}</td>
        <td>{{ oneInstance.telephone }}</td>
        <td>
          <ooredoo-wx-companies :employee="oneInstance"/>
        </td>
      </tr>
      </tbody>
      <tfoot>
  ...
  <ooredoo-create-client-company-popup :isCreateEmployeePopupShown="!isCreateEmployeePopupShown"  @cancelFunction="()=>{isCreateEmployeePopupShown = !isCreateEmployeePopupShown}" />
...
import OoredooCreateClientCompanyPopup from "@/views/ooredoo/Companies/ooredooCreateClientCompanyPopup.vue";
import OoredooWxCompanies from "@/views/ooredoo/Companies/ooredooWxCompanies.vue";
...
onMounted(async () => {
  try {
    const response = await axios.get('https://pfe.ramzi-issiakhem.com/api/v1/companies/client?perPage=50&page=1');
...
const filteredInstance = computed(() => {
  return instance.value.filter(item => {
    return item.nom.toLowerCase().includes(search.value.toLowerCase())
  })
})
