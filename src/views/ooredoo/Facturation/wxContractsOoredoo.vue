<template>
  <div class="sala">
    ...
    <EditOoredooContractPopup  :employee="selectedEmployee" v-if="selectedEmployee !== null" @cancelFunction="()=>{editEmployeePopupisShown = !editEmployeePopupisShown;}" :isShown="editEmployeePopupisShown" />

    <ooredoo-user-info-popup  :employee="selectedEmployee"
...
import OoredooUserInfoPopup from "@/views/ooredoo/Employees/ooredooUserInfoPopup.vue";
import EditOoredooEmployeePopup from "@/views/ooredoo/Employees/EditOoredooEmployeePopup.vue";
import EditOoredooContractPopup from "@/views/ooredoo/Facturation/EditOoredooContractPopup.vue"; // import the Popup component
...
async function removeEmployee(employee) {
  try {
    ...
    const deleteUrl = `https://pfe.ramzi-issiakhem.com/api/v1/user/delete/${employee.id_ooredoo_user}`;
    const response = await axios.delete(deleteUrl);
    // Handle the response as needed
    console.log(response.data);

    alert(employee.nom + ' ' + employee.prenom + 'successfully removed.');
