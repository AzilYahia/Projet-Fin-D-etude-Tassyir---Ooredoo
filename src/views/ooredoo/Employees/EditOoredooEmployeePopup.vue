        ...
        <form @submit.prevent="saveEmployee" style="height: 80%; width: 100%;">
          <div class="fields">
            <div class="form-group" >
              ...
              <input id="name" type="text" v-model="employee.prenom" required>
            </div>
            <div class="form-group">
              <label for="other-fields">Nationalite</label>
              <input id="other-fields" v-model="employee.nationalite">
            </div>
            <div class="form-group">
              <label for="role">Etat</label>
              <input id="role" type="text" v-model="employee.etat" required>
            </div>
            <div class="form-group">
              <label for="role">Type utilisateur</label>
              <input id="role" type="text" v-model="employee.typeUtilisateur" required>
            </div>
          </div>
          <div class="buttons">
...
export default {
  ...
  methods: {
    ...
    async saveEmployee() {
      try {
        const patchUrl = `https://pfe.ramzi-issiakhem.com/api/v1/ooredoo/user/edit/${this.employee.id_ooredoo_user}`;
        const response = await axios.patch(patchUrl, {
          ...
          typeUtilisateur: this.employee.typeUtilisateur
        });
