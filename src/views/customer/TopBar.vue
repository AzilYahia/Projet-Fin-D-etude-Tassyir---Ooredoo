...
<!--    <div class="image"></div>-->
        <v-avatar color="#df0327" size="60">
                    <span class="text-h4" style="color: white" v-if="localStorageUser != null">
...
function goToSettings(){
    routerr.push('/customer/settings')
}
function logOut(){
    logOutIsHidden.value = !logOutIsHidden.value
}
function goToProfile(){
    routerr.push('/customer/myProfile')
}
function goToMessages(){
    routerr.push('/customer/messages')
}

const loading = ref(true); // add a loading state
const instance = ref([]);

const clientUserId = localStorageUser?.id_client_user
onMounted(async () => {
  try {
    const response = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/user/client/${clientUserId}`);
