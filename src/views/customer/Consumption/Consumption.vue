...
<!--        </v-card-actions>-->
    ...
    >
      ...
      <v-card title="Your limit"  height="100%">
...
onMounted(async () => {
  try {
    ...
    const companyConsumption = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/company/consumption/${clientCompanyId}?begin=${startOfMonth}&end=${endOfMonth}`);
    instance.value = companyConsumption.data.data;
    // ^ : {
    // "data": {
    //   "date_debut": "2023-04-22",
    //       "date_fin": "2023-05-25",
    //       "credit": 50030,
    //       "internet": 40
    // },
    // "status": 200
  // }
    const clientPhoneLineId = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/company/user/line/${localStorageUser.id_client_user}`);
    if(clientPhoneLineId) {
      const clientConsumption = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/client/user/consumption/${clientPhoneLineId}?begin=${startOfMonth}&end=${endOfMonth}`);
      ...
      const clientYesterday = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/client/user/consumption/yesterday/${clientPhoneLineId}`);
    ...
    console.log(error);
    console.log('client phone line id is null');
    loading.value = false;
  }
});
