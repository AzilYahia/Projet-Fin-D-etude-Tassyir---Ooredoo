...
<!--                <td>{{ consumptionData[oneInstance.id] }}</td>-->
                ...
                <td class="aso" v-if="oneInstance.utilisateur == null">
                  None
<!--                  {{oneInstance.utilisateur.prenom}}-->
                </td>

              <td class="aso" v-else>
                {{oneInstance.utilisateur.nom}}
              </td>
                ...
                <td >
                    <wx-call-lines :employee="oneInstance"/>
                </td>
            </tr>
...
onMounted(async () => {
    try {
        const response = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/company/client/lines/${clientCompanyId}`);
        instance.value = response.data.data;
        const usersResponse = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/users/${clientCompanyId}`);
        userData.value = usersResponse.data.data;
         for (let i = 0; i < instance.value.length; i++) {
            ...
            instance.value[i].id_line_client
                +'?begin=2021-01-01');
...
console.log(filteredInstance)

const fetchConsumptionData = computed(async () => {
    const result = {};
    for (const oneInstance of filteredInstance.value) {
        const response = await axios.get(`https://pfe.ramzi-issiakhem.com/api/v1/client/user/consumption/${oneInstance.id_line_client}?begin=2021-01-01`);
...
async function consumptionconnexions ()   {
    ...
    for ( const i of filteredInstance.value) {
       ...
       //  console.log(wx)
        // console.log(i)
        const clientConsumptionResponse = await axios.get('https://pfe.ramzi-issiakhem.com/api/v1/client/user/consumption/'+ i.id_line_client +'?begin=2021-01-01');
        clientconsumption.value.push(clientConsumptionResponse.data.data);
        // console.log(clientconsumption.value)
...
async function consumptionconnexion (wx)   {
    const clientConsumptionResponse = await axios.get('https://pfe.ramzi-issiakhem.com/api/v1/client/user/consumption/'+ wx.id_line_client +'?begin=2021-01-01');
