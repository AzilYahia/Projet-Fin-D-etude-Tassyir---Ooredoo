<template>
  <div class="sala">
    ...
    <EditPhoneLinePopup  :employee="selectedEmployee" v-if="selectedEmployee !== null" @cancelFunction="()=>{editEmployeePopupisShown = !editEmployeePopupisShown;}" :isShown="editEmployeePopupisShown" />

    <Popup  :employee="selectedEmployee"
...
import {clientCompanyId} from "@/utils/localStorageData"; // import the Popup component
...
const numerotelephone=[`${employee.numeroTelephone}`];
console.log(employee.id_line_client)
// setTimeout(() => {console.log(employee)},4000)
async function removeEmployee(employee) {
  try {
    const deleteUrl = `https://pfe.ramzi-issiakhem.com/api/v1/company/client/delete/lines`;
    const response = await axios.delete(deleteUrl,{
      data:{      lignes: numerotelephone,
    ...
    alert(employee.numeroTelephone + ' ' + 'successfully removed.');
    ...
    console.error(error.config); // Log the request configuration
  }
