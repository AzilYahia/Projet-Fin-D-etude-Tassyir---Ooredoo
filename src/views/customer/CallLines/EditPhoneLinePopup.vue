        ...
        <form @submit.prevent="saveEmployee" style="height: 80%; width: 100%;">
          <div class="fields">
            <div class="form-group" >
              ...
              <input id="name" type="text" v-model="employee.etatLigne" required>
            </div>
          </div>
          <div class="buttons">
...
export default {
  ...
  methods:{
    ...
    async saveEmployee() {
      try {
        const patchUrl = `https://pfe.ramzi-issiakhem.com/api/v1/user/client/line/suspend/${this.employee.id_line_client}`;
        const response = await axios.patch(patchUrl, {
          state: this.employee.etatLigne,
        });
